...
class Habits:
    ...
    def __init__(self, master, username):
        ...
        self.master.geometry("479x569")
        self.master.iconbitmap('assets/main_logo_icon.ico')
        ...
        while master_column < 10:
            self.master.columnconfigure(master_column, minsize=25)
            master_column += 1
        ...
        self.frame_welcome.grid(row=0, column=0, rowspan=2, columnspan=10)
        ...
        while welcome_column < 10:
            self.frame_welcome.columnconfigure(welcome_column, minsize=25)
            welcome_column += 1

        # *Tab Button Frame Construction ------------------------------

        self.frame_tab = ttk.Label(
            self.master, text = 'My Habits', style='WhiteOnPink.TLabel')
        self.frame_tab.configure(anchor="center")
        self.frame_tab.grid(row=2, column=0, rowspan=1, columnspan=10, sticky = 'news', ipady=12)

        # Define grid 1 x 21
        tab_rows = 0
        while tab_rows < 1:
            self.frame_tab.rowconfigure(tab_rows, minsize=25)
            tab_rows += 1

        tab_column = 0
        while tab_column < 21:
            self.frame_tab.columnconfigure(tab_column, minsize=25)
            tab_column += 1
        ...
        self.frame_top1 = ttk.Frame(
            self.master, relief=FLAT, style='TFrame')
        self.frame_top1.grid(row=3, column=0, rowspan=1, columnspan=10)
        ...
        while top1_column < 21:
            self.frame_top1.columnconfigure(top1_column, minsize=25)
            top1_column += 1
        ...
        self.frame_main_habits = ttk.Frame(
            self.master, relief=FLAT, style='TFrame')
        self.frame_main_habits.grid(row=4, column=0, rowspan=5, columnspan=9,sticky='news')

        # Define grid 7 x 16
        info1_row = 0
        while info1_row < 7:
            self.frame_main_habits.rowconfigure(info1_row, minsize=22.5)
            info1_row += 1

        info1_column = 0
        while info1_column < 16:
            self.frame_main_habits.columnconfigure(info1_column, minsize=13.5)
            info1_column += 1
       ...
       # Define grid 3 x 21
        info2_row = 0
        while info2_row < 3:
            self.frame_clocked_habit.rowconfigure(info2_row, minsize=25)
            info2_row += 1
        ...
        info4_column = 0
        while info4_column < 10:
            self.frame_create_habit.columnconfigure(
                info4_column, minsize=25)
            info4_column += 1
        ...
        self.frame_footer.grid(row=10, column=0, rowspan=1, columnspan=10)

        # Define grid 1 x 10
        top2_rows = 0
        while top2_rows < 1:
            self.frame_footer.rowconfigure(top2_rows, minsize=25)
            top2_rows += 1
        ...
        custom_named_label = self.user_first_name
        ...
        self.top_logo.grid(row=0, column=0, rowspan=2)
        self.header_label.grid(row=0, column=1, rowspan=2,
                               columnspan=2, sticky='w')
        self.date_label.grid(row=0, column=4, rowspan=2, columnspan=5,sticky='w')
        self.signout_button.grid(row=0, column=9, rowspan=2, sticky='e', padx=5)
        ...
        for habits in self.today_data:
            ...
            self.progress_bar.append(ttk.Progressbar(
                self.frame_habits_cont, orient=HORIZONTAL, length=80, mode='determinate', maximum=habits[1], value=habits[3], style='Pink.Horizontal.TProgressbar'))
            self.progress_bar[self.habit_counter].grid(
                row=self.habit_counter, column=9, columnspan=3, sticky='we', padx=10, ipadx=10)
        ...
        #Label Break Baars
        self.space_label_white = ttk.Label(self.frame_main_habits_button,
                                           text=' ',
                                           width=80)
        
        self.space_label_green = ttk.Label(self.frame_main_habits_button,
                                           text=' ',
                                           background='#294a00',
                                           width=80)
        ...
        self.manage_habit_button = ttk.Button(self.frame_main_habits_button,
                                           text='Manage My Habits',
                                           style='GreenButton2.TButton',
                                           width=80,
        ...
        self.add_habit_button = ttk.Button(self.frame_main_habits_button,
                                           ...
                                           style='GreenButton2.TButton',
                                           width=80,
                                           command=lambda: self.open_add_habits(self.username))

        # Button Stats Clocked Habits
        self.w_stats_button = ttk.Button(self.frame_main_habits_button,
                                         text='Habit Stats',
                                         style='GreenButton2.TButton',
                                         width=80,
        ...
        # Geometrtic Distribution
        # *PANEL 2
        self.space_label_white.grid(row=2, column=0, rowspan=1, columnspan=10, sticky = 'news')
        self.space_label_green.grid(row=3, column=0, rowspan=1, columnspan=10, sticky = 'news')
        self.manage_habit_button.grid(row=4, column=0, rowspan=1, columnspan=10, sticky = 'news', ipady=7)
        self.add_habit_button.grid(row=5, column=0, rowspan=1, columnspan=10, sticky = 'news', ipady=7)
        self.w_stats_button.grid(row=6, column=0, rowspan=1, columnspan=10, sticky = 'news', ipady=7)
        ...
        self.entry_name = ttk.Entry(
            self.frame_create_habit, width=10, font=('Century Gothic', 10))
        ...
        self.spinbox_repetition = ttk.Spinbox(
            self.frame_create_habit, from_=1, to=999, textvariable=self.repetition_value, width=3)
    ...
    def manage_window(self, root2):
        ...
        self.root2.title('HealthTrack')
        self.root2.resizable(False, False)
        self.root2.geometry("700x600")
        self.root2.iconbitmap('assets/main_logo_icon.ico') 
        self.root2.configure(background='#c2d4ba')
        ...
        while deletion_header_column < 10:
        ...
        self.active_header_label.grid(
            row=0, column=0, rowspan=1, columnspan=10, sticky='nesw')
        ...
        self.scroll_canvas_2 = Canvas(
            self.frame_deletion_panel, width=600, height=305, highlightthickness=0, relief='ridge', bg='#c2d4ba')
    ...
    def open_add_habits(self, username):
        '''This method loads the Manage Habit window. The Manage Habit Window
        allows the user to manage active habits'''

        self.root2 = Toplevel(self.master)
        self.add_window(self.root2)

    def add_window(self, root2):
        '''This method is only callable only if OPEN_MANAGE_HABITS has been called. 
        This method manages all the gui and methods for the Manage Habit Window
        The Manage Habit Window allows the user to manage active habits'''
        ...
        # Labels Create Habit
        self.create_habit_label = ttk.Label(self.root2,
                                            text='New Habit',
                                            style='WhiteOnPink.TLabel',
                                            anchor="center")
        # Habit Name Label
        self.name_label = ttk.Label(
            self.root2, text='Habit Name:', style='BoldTextOnYellow.TLabel')

        # Entry for Habit Name
        self.entry_name = ttk.Entry(
            self.root2, width=13, font=('Century Gothic', 10))

        # Daily Repetitions Name Label
        self.repetition_label = ttk.Label(
            self.root2, text='Daily Goal:', style='BoldTextOnYellow.TLabel')

        # Spinbox for Daily Repetitions
        self.repetition_value = IntVar()
        self.repetition_value.set(1)
        self.spinbox_repetition = ttk.Spinbox(
            self.root2, from_=1, to=999, textvariable=self.repetition_value, width=3)

        # Radiobutton for Monday
        self.monday_day = StringVar()
        self.monday_radiobutton = ttk.Checkbutton(
            self.root2, text='Mon.', variable=self.monday_day, onvalue='Monday', offvalue='', style='TCheckbutton')

        # Checkbutton for Tuesday
        self.tuesday_day = StringVar()
        self.tuesday_radiobutton = ttk.Checkbutton(
            self.root2, text='Tue.', variable=self.tuesday_day, onvalue='Tuesday', offvalue='', style='TCheckbutton')

        # Checkbutton for Wednesday
        self.wednesday_day = StringVar()
        self.wednesday_radiobutton = ttk.Checkbutton(
            self.root2, text='Wed.', variable=self.wednesday_day, onvalue='Wednesday', offvalue='', style='TCheckbutton')

        # Checkbutton for Thursday
        self.thursday_day = StringVar()
        self.thursday_radiobutton = ttk.Checkbutton(
            self.root2, text='Thu.', variable=self.thursday_day, onvalue='Thursday', offvalue='', style='TCheckbutton')

        # Checkbutton for Friday
        self.friday_day = StringVar()
        self.friday_radiobutton = ttk.Checkbutton(
            self.root2, text='Fri.', variable=self.friday_day, onvalue='Friday', offvalue='', style='TCheckbutton')

        # Checkbutton for Saturday
        self.saturday_day = StringVar()
        self.saturday_radiobutton = ttk.Checkbutton(
            self.root2, text='Sat.', variable=self.saturday_day, onvalue='Saturday', offvalue='', style='TCheckbutton')

        # Checkbutton for Sunday
        self.sunday_day = StringVar()
        self.sunday_radiobutton = ttk.Checkbutton(
            self.root2, text='Sun.', variable=self.sunday_day, onvalue='Sunday', offvalue='', style='TCheckbutton')
        ...
        # MORNING IMAGE
        self.morning_image_add_habit = self.morning_image.subsample(2, 2)
        self.morning_button = ttk.Button(self.root2,
                                         image=self.morning_image_add_habit,
                                         style='YellowButton.TButton',
                                         command=self.morning_action)
        ...
        # EVENING IMAGE
        self.evening_image_add_habit = self.evening_image.subsample(2, 2)
        self.evening_button = ttk.Button(self.root2,
                                         image=self.evening_image_add_habit,
                                         style='YellowButton.TButton',
                                         command=self.evening_action)
        ...
        # ALL DAY IMAGE
        self.all_day_image_add_habit = self.all_day_image.subsample(2, 2)
        self.all_day_button = ttk.Button(self.root2,
                                         image=self.all_day_image_add_habit,
                                         style='YellowButton.TButton',
                                         command=self.all_time_action)
        ...
        # AFTERNOON IMAGE
        self.afternoon_image_add_habit = self.afternoon_image.subsample(2, 2)
        self.afternoon_button = ttk.Button(self.root2,
                                           image=self.afternoon_image_add_habit,
                                           style='YellowButton.TButton',
                                           command=self.afternoon_action)

        # Button Add Habit
        self.add_habit_button = ttk.Button(self.root2,
                                           text='Add',
                                           style='GreenButton.TButton',
                                           command=self.submit_new_habit)
        ...
        self.create_habit_label.grid(
            row=0, column=0, rowspan=3, columnspan=18, sticky='nesw', ipady=12)

        self.name_label.grid(row=3, column=0, rowspan=2,
                             columnspan=3, sticky='w', padx=10, pady=3)
        self.entry_name.grid(row=5, column=0, rowspan=2,
                             columnspan=16, sticky='nesw', padx=10, pady=3)
        self.repetition_label.grid(
            row=7, column=0, rowspan=2, columnspan=3, sticky='w', padx=10, pady=3)
        self.spinbox_repetition.grid(
            row=7, column=3, rowspan=2, columnspan=4, sticky='news', padx=10, pady=3)
        self.monday_radiobutton.grid(
            row=11, column=0, rowspan=1, columnspan=2, sticky='e')
        self.tuesday_radiobutton.grid(
            row=11, column=2, rowspan=1, columnspan=2, sticky='w')
        self.wednesday_radiobutton.grid(
            row=11, column=4, rowspan=1, columnspan=2, sticky='w')
        self.thursday_radiobutton.grid(
            row=11, column=6, rowspan=1, columnspan=2, sticky='w')
        self.friday_radiobutton.grid(
            row=11, column=8, rowspan=1, columnspan=2, sticky='w')
        self.saturday_radiobutton.grid(
            row=11, column=10, rowspan=1, columnspan=2, sticky='w')
        self.sunday_radiobutton.grid(
            row=11, column=13, rowspan=1, columnspan=2, sticky='w')

        self.morning_button.grid(row=13, column=4, rowspan=4, columnspan=3)
        self.afternoon_button.grid(row=13, column=7, rowspan=4, columnspan=3)
        self.evening_button.grid(row=13, column=10, rowspan=4, columnspan=3)
        self.all_day_button.grid(row=13, column=13, rowspan=4, columnspan=3)

        self.add_habit_button.grid(
            row=14, column=0, rowspan=3, columnspan=3, sticky='nesw')